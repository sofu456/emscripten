...
(You may need this answer if you see an error saying something like ``native function `x` called before runtime intialization``, which is a check enabled in ``ASSERTIONS`` builds.)
